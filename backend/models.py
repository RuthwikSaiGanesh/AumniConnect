...
class Student(db.Model):
    ...
    course = db.Column(db.String(100), nullable=True)  # Course the student is enrolled in
    year = db.Column(db.String(20), nullable=True)  # Year of study
...
class Alumni(db.Model):
    ...
    """
    id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='CASCADE'), primary_key=True)
    graduation_year = db.Column(db.String(10), nullable=True)  # Year of graduation
...
class Professor(db.Model):
    ...
    """
    id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='CASCADE'), primary_key=True)
    department = db.Column(db.String(100), nullable=True)  # Department the professor belongs to
